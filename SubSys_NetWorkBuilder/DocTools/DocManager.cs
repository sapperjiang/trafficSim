...
//                if (!Empty(s))
 ...
 //           }
        ...
        public bool ExportToJpg()
        {
            ...
            saveFileDialog2.Filter = "Image file (*.jpg)|*.jpg|All Files (*.*)|*.*";

            if (!Empty(newFileName))
            {
                saveFileDialog2.InitialDirectory = Path.GetDirectoryName(newFileName);
                saveFileDialog2.FileName = Path.GetFileName(newFileName);
            }
            else
            {
                saveFileDialog2.InitialDirectory = fileDlgInitDir;
                saveFileDialog2.FileName = newDocName;
            }

            DialogResult res = saveFileDialog2.ShowDialog(frmOwner);

            if (res != DialogResult.OK)
                return false;

            newFileName = saveFileDialog2.FileName;
            // Write the data
            try
            {
                using (Stream stream = new FileStream(
                           newFileName, FileMode.Create, FileAccess.Write))
                {
                    ...
                    if (ExportEvent != null)        // if caller subscribed to this event
                    {
                        ExportEventArgs args = new ExportEventArgs(newFileName, stream, true);

                        // raise event
                        ExportEvent(this, args);
            ...
            { 
                //return HandleExportException(e, newFileName);
                //return false;
            }
            return true;
        }
...
//            key.SetValue(registryValue, fileDlgInitDir);
        ...
        /// <param name="fileName"></param>
        /// <returns></returns>
        private bool HandleExportException(Exception ex, string fileName)
        {
            MessageBox.Show(frmOwner,
                "Export File operation failed. File name: " + fileName + "\n" +
                "Reason: " + ex.Message,
                Application.ProductName);
    ...
    #region Class ExportEventArgs

    /// <summary>
    /// Export to jpg event arguments.
    /// Used in events raised from DocManager class.
    /// Class contains name of file and result of Open operation.
    /// </summary>
    public class ExportEventArgs : System.EventArgs
    {
        private string fileName;
        ...
        public ExportEventArgs(string fileName, Stream stream, bool success)
        {
            this.fileName = fileName;
            this.stream = stream;
