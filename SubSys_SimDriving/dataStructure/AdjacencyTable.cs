...
namespace SubSys_SimDriving.TrafficModel
{
    ...
    {
        Dictionary<T,XNode> dicXNode; //图的顶点集合
        /// <summary>
        /// 使用SimDrivingContext 仿真上下文初始化保存路段节点的字典
        /// </summary>
        internal AdjacencyList(Dictionary<T,XNode> dic)
        {
            dicXNode = dic;
        } //构造方法
        private AdjacencyList() //私有构造防止外部初始化指定容量的构造方法
        { }

        internal void AddXNode(T key,XNode value) /*添加?个顶点 */
        {   //不允许插入重复值
            if (Contains(key))//哈希值一致就认为顶点一致
            {
                throw new ArgumentException("插入了重复顶点！");
            }
            dicXNode.Add(key,value);
        }
        internal void RemoveXNode(T key)
        {
            dicXNode.Remove(key);
        }

        internal int XNodeCount
        {
            get { return dicXNode.Count; }
        }

        internal int WayCount
        {
            get
            {
                int iCount = 0;
                foreach (XNode item in dicXNode.Values)
                {
        ...
        internal bool Contains(T key) //查找图中是否包含某RoadNode,key是根据对象位置进行的哈希散列值
        {
            if (key != null)
            {
                return dicXNode.ContainsKey(key);
            }
            return false; 
        }
        internal XNode Find(T key) //查找指定项并返回
        {
            if (key != null)
            {
                if(!dicXNode.ContainsKey(key)){
                throw new Exception("无法找到没有添加的RoadNode节点");
                }
                return dicXNode[key] as XNode;
            }
            return null;
        }
