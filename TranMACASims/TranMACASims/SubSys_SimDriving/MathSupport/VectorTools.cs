...
namespace SubSys_SimDriving.MathSupport
{
    ...
    internal static class VectorTools
    {
        ...
        /// <param name="re"></param>
        /// <returns></returns>
        internal static MyPoint getVector(RoadEdge re)
        {
            return new MyPoint(re.to.Postion.X - re.from.Postion.X, re.to.Postion.Y - re.from.Postion.Y);
        }
        ...
        internal static SinCos getSinCos(MyPoint mpBaseVector, MyPoint mpVector)
        {
            ...
            int iPos = VectorTools.getVectorPos(mpBaseVector, mpVector);
            
            if (-0.708 <= dCosineValue && dCosineValue < 0.708)
            {///判断y位于基向量的上方还是下方
                if (iPos < 0)//-45 270 +45度
                {
